  ...
  if (postList.length == 0) {
    return (
      <div className="w-full text-center">
        Este usuario aún no ha posteado nada
      </div>
    )
  }
      ...
      {Object.keys(postList)
        ...
        .map((element, index) => {
          ...
          return (
            <div
              key={element.id}
              className={`relative flex flex-col w-full border border-purple-800 rounded-2xl bg-gradient-to-br ${prog == "Started" ? `from-blue-200` : prog == "Finished" ? `from-emerald-400` : `from-yellow-200`} via-slate-200 to-slate-200 py-3 px-4 space-y-2`}
            >
                  ...
                  <p className="font-medium">
                    ...
                    {prog == "Started"
                      ...
                      : prog == "Finished"
                        ? "terminó"
                        : "quiere " + data.action}{" "}
                    <strong>{element.metadata.title}</strong>
                  </p>
