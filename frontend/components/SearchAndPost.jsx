      ...
      {media ? (
        <div className="flex flex-col w-full space-y-2">
          <div className="w-full space-y-2">
            ...
            <div className="flex flex-row items-center justify-between bg-purple-300 rounded-lg py-2 px-4">
              <div className="flex flex-row items-center space-x-2">
                {media.image ? (
                  <img width={40} src={media.image} />
                ) : (
                  <div className="bg-slate-500 h-14 w-9 flex items-center justify-center text-slate-200"></div>
                )}
          ...
          <div className="flex flex-col space-y-2">
            <label className="font-medium bg-clip-text text-transparent bg-gradient-to-r from-purple-800 to-slate-900">
              Tell us what you thought of it:
            </label>
            <textarea
              value={textAreaValue}
              onChange={(e) => setTextAreaValue(e.target.value)}
              className="border border-purple-800 rounded-2xl w-full p-2 font-medium"
            />
          </div>
          <div className="flex w-full items-center justify-center">
            <button
              onClick={handlePost}
              disabled={loading}
              className="btn-sm w-60 px-4 py-2 text-slate-300 hover:text-white transition duration-150 ease-in-out group [background:linear-gradient(theme(colors.slate.900),_theme(colors.slate.900))_padding-box,_conic-gradient(theme(colors.slate.400),_theme(colors.slate.700)_25%,_theme(colors.slate.700)_75%,_theme(colors.slate.400)_100%)_border-box] relative before:absolute before:inset-0 before:bg-slate-800/30 before:rounded-full before:pointer-events-none"
            >
              {loading ? (
                loadingSvg()
              ) : (
